...
8. **Commit focused units**:
   ...
   - **For substantive PRs, run Tier-1 review on the commit you just made (before push)**:
     ...
     - Simplification is part of Tier-1's focus (the commit-review prompts carry a reuse / dead code / cruft focus item, so it runs every time Tier-1 runs); `/simplify` remains an optional interactive extra in Claude Code sessions.
...
**Cap the consistency self-check.** The consistency self-check (`make review-plan-consistency-by-claude`, run at the `N.5` cadence) exists to catch cross-section contradictions a *fold* just introduced — it is not a wording-polish loop. Cap it at ~2 passes per integer iteration. If a pass surfaces only cosmetic or self-inflicted nits — a heading the previous fold reworded, an enumeration count the fold itself changed, phrasing drift — stop and accept the state, or carry it to the next integer cross-review (which catches anything that actually matters). The signal you've hit the cap: each fold spawns the *next* nit instead of resolving a contradiction, and no finding would change what the implementation does. This is the same-class-regeneration trigger above applied to the self-check — re-running the same static check on self-inflicted churn is not convergence. (The plan-review-loop guardrails plan's own self-check ran 5 passes on cosmetic drift before being capped by hand — the instance this rule generalizes.)
...
Claude Code sessions (only) can run Tier-1 via a fresh `general-purpose` subagent instead of the Makefile target — useful when you want to ask follow-up questions interactively. The prompt texts live in `prompts/` — the same single source the Makefile recipes build from via `scripts/render-review-prompt.py`, so the subagent path can never drift from the recipe path. Substitute your commit SHA for `{COMMIT_REF}` (get it from `git log -1 --pretty=%H`). **Pick ONE variant** matching your situation:

### Variant A — If you have a plan to check drift against

Read `prompts/commit-review-plan-bound.txt` and substitute BOTH `{COMMIT_REF}` and `{PLAN_FILE}` (your active plan path, e.g. `docs/plans/2026-05-17-foo.md`); pass the result as the subagent prompt.

### Variant B — If you have no plan binding (code-correctness only)

Read `prompts/commit-review-unbound.txt` and substitute only `{COMMIT_REF}`. The unbound prompt tells the reviewer to limit findings to code-correctness and not to infer a plan file by mtime.

(For non-Claude-Code sessions, just run `make review-commit-by-claude PLAN_FILE=docs/plans/<active>.md` — or `make review-commit-by-codex PLAN_FILE=...` when Codex is the implementer; Tier-1 uses the same AI as the implementer, cross-AI review is Tier-2's job. The Makefile builds its prompt from the same `prompts/` files. Omit `PLAN_FILE` for the unbound variant.)
