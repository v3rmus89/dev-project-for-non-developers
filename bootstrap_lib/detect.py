...
def _first_requirements_match(out_dir: Path) -> str | None:
    ...
    etc. — the `requirements*.txt` wildcard contract.
    """
    # Filter to files only — a directory named `requirements-test.txt`
    # shouldn't count as a pip marker.
...
def detect_package_manager(out_dir: Path | str | None) -> DetectionResult:
    ...
    Accepts `Path`, `str`, or `None` — `args.out` is a `str` from argparse
    and can be `None` in dry-run/diff modes.
    ...
    # `uv.lock` shouldn't false-positive.
    ...
    # key — `backend` is the wrong TOML key and would miss real uv
    # adoption cases).
    #
    # `isinstance(..., dict)` guard prevents crash if `build-system` is
    # a scalar (would AttributeError on `.get`).
    if isinstance(pyproject, dict):
    ...
    # so a pip marker is preserved when pyproject is malformed.
    ...
    # so the CLI's advisory can print the explicit override hint —
    # returning a manager here would make the CLI's ambiguous-advisory
    # branch dead code).
    if isinstance(pyproject, dict):
