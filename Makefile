...
review-plan-by-codex:	## Codex skeptical review of a plan file (PLAN_FILE=docs/plans/foo.md [ITERATION=N])
	...
	if [ "$(THREAD_MODE)" = "continue" ]; then \
	  if [ -f "$(THREAD_FILE)" ]; then \
	    ...
	    if OUT=$$($(CURDIR)/scripts/run-with-clean-env.py -- codex exec resume "$$SESSION_ID" --output-last-message "$(PLAN_REVIEW_OUT_CODEX)" "$$PROMPT" 2>&1); then \
	      echo "$$OUT"; \
	    else \
	      rc=$$?; \
	      echo "$$OUT" >&2; \
	      if printf '%s' "$$OUT" | grep -q 'no rollout found for thread id'; then \
	        ...
	        $(CURDIR)/scripts/run-with-clean-env.py -- codex exec -C "$(CURDIR)" --sandbox read-only --color never --output-last-message "$(PLAN_REVIEW_OUT_CODEX)" "$$PROMPT"; \
	      else \
	        exit $$rc; \
	      fi; \
	    fi; \
	    ...
	    $(CURDIR)/scripts/run-with-clean-env.py -- codex exec --json -C "$(CURDIR)" --sandbox read-only --color never --output-last-message "$(PLAN_REVIEW_OUT_CODEX)" "$$PROMPT" > "$(THREAD_JSONL_FILE)" \
