...
review-plan-fact-check-by-codex:	## Pre-pass: fact-check active plan sections against declared fact roots (PLAN_FILE=...)
	...
	@PROMPT="$$(PLAN_FILE="$(PLAN_FILE)" VERIFICATION_JSON="" VERIFICATION_JSON_FILE="$(FACT_CHECK_VERIFY_OUT)" $(CURDIR)/scripts/render-review-prompt.py $(CURDIR)/prompts/fact-check-interpret.txt)" || exit $$?; \
	$(CURDIR)/scripts/run-with-clean-env.py -- \
	  codex exec \
	    -C "$(CURDIR)" \
	    --sandbox read-only \
