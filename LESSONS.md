...
**Trigger**: Scrubbing private project names before making the repo public, I built the substitution table from what `git grep -i boxette` / `call-details` matched and declared it clean — 428 replacements, residual grep empty. Tier-2 review (retroactive, on merged PR #54) found three classes still live on the now-public repo: (1) **absolute paths** — I replaced `~/Desktop/Code/Boxette/call-details` but not the `/Users/sandeep/Desktop/Code/...` form, leaving 25 occurrences that leak the username and home topology; (2) **identifier variants** — `boxette_calls` was in the table but `acme_chats`, `call_details` and `CallDetails` were not, so 14 survived; (3) **adjacent secrets of the same kind** — a real customer-facing branch name and SHA in `docs/trial-report-pr7.md`, in a file whose own privacy note forbids customer content. The residual grep was empty only because it searched for the same tokens the table already handled — it could not fail.
