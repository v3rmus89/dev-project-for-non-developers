...
| fa7b2c8 | Tier-2 fix — guard `--interactive` against a `None` `sys.stdin` in `main()` before the intake block (exit 2 with the standard non-TTY message, not an uncaught `AttributeError`); new test | none — closes a Codex Tier-2 P2 (the iter-5 `sys.stdin is not None` guard covered `_should_run_intake`'s auto-trigger branch but not the explicit `--interactive` branch) | none — Tier-1 clean (0 imp-3 / 0 imp-2; 2 imp-1 polish accepted as-is) |
