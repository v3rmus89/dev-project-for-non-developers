...
- Retarget the shared-template SCAN tests (Tier-2 codex P1, verified): the six
  ...
  `test_no_acme_isms_in_shared_templates` call `get_template()` on each — after
  ...
  Jinja templates, so "renders without undefined" and "no acme-isms in a
