...
- **Substitution variables**: `project_name`, `language`, `python_version`, `enable_smoke`, `github_owner`, `github_repo`, `github_review_mode`
...
| existed_before=true (overwritten) | neither | SKIP with warning "user edit or partial write detected; left in place". Conservative — never clobbers user edits. |
...
2. **Active-surface consistency check** (closes Codex iter-17 finding #4) — manual grep before opening the PR:
   ```bash
   grep -n "install-hooks\|install_hooks\|pre_commit" docs/plans/2026-05-15-skill-pr1-minimal-python-bootstrap.md
   ```
   ...
   - **legacy/backlog**: lives inside a clearly-marked legacy section or evidence-table row (historical record, OK)
   - **forbidden**: appears in an ACTIVE PR #1 surface (CLI flag list, manifest schema, deployment-contract check order, test list, docs/usage.md) — must be cleaned up
   Forbidden matches → fix before opening the PR. Same check for any other "removed feature" in future iterations: the surface-drift failure mode this catches (iter-17 finding #1) is exactly what the triage rule's "fold completely" obligation prevents structurally — but doing the grep is the safety net.
...
6. **`make doctor` smoke** (closes Codex iter-17 finding #6): `make help` lists `doctor`; running `make doctor` with all core tools on PATH exits 0; running it with `make` shadowed off PATH (via `PATH=/usr/bin:/bin make doctor` from a controlled env) exits non-zero with a clear "missing dep" message. Optional-tool absence (`claude` / `codex` not on PATH) produces an advisory note but does NOT fail
