...
**Tier-1 plan-file binding** (closes Codex iter-3 #4 + iter-6 #2): the macro signature becomes `tier1_prompt(commit_ref, plan_file=None)`. ALL THREE Tier-1 entrypoints honor the binding:

1. **`review-commit-by-{codex,claude}` Makefile targets** accept `PLAN_FILE=docs/plans/<file>.md` and pass to the macro.
2. **CONTRIBUTING.md.tmpl's subagent template** (the in-session Claude Code path) also takes `<PLAN_FILE>` as a placeholder alongside `<SHA>`; the rendered subagent prompt invokes `{{ tier1_prompt('<SHA>', '<PLAN_FILE>') }}`. Driver substitutes both.
3. **In-session ad-hoc subagent** (no make, no CONTRIBUTING template): if no PLAN_FILE provided, the rendered prompt's fallback text says "no specific plan provided; this review CANNOT check plan-impl drift — limit findings to code-correctness only" (closes Codex iter-6 #2 — earlier wording allowed silent unbound plan-drift review).

When PLAN_FILE IS set: the prompt explicitly says "check this commit against `docs/plans/<file>.md`'s plan body; flag any deviation". When unset: prompt says "no plan binding; limit findings to code-correctness, do not infer a plan file by mtime". Avoids the reviewer picking the wrong plan from a multi-plan repo. CONTRIBUTING.md step-9 sub-bullet now reads `make review-commit-by-claude PLAN_FILE=docs/plans/<active>.md` (PLAN_FILE listed as recommended, with the unbound-fallback caveat documented).
...
Extend the `tier1_prompt(commit_ref)` macro in `shared/Makefile.review.tmpl` to also instruct the reviewer to propose an impl-log row:
...
| `tests/test_shared_templates.py` (extend) | (a) `tier1_prompt` macro contains the impl-log-row instruction (substring assertion). (b) `tier1_prompt` byte-identity test (existing) — extend to **parameterized** over `PLAN_FILE` argument: assert both the `commit_ref='HEAD'` + no-PLAN_FILE variant AND the `commit_ref='HEAD', plan_file='docs/plans/x.md'` variant render byte-identically between Makefile recipe and CONTRIBUTING.md template (closes self-check 4.5 #6 — the new optional arg means byte-identity must be tested across both states, not just default). (c) **NEW**: `shared/CONTRIBUTING.md.tmpl` step-9 sub-bullet contains the mandate wording — substrings: "append" + ("impl-log" or "Implementation log"). Dogfood `CONTRIBUTING.md` same assertion (closes Codex iter-1 #4 + self-check #5). (d) **NEW**: `shared/CONTRIBUTING.md.tmpl` `/simplify` sub-bullet contains the gating wording — substrings: "Claude Code" + "skip" + "optional" (closes self-check 5.5 #4 — Bucket E promised the doc-test but Bucket F hadn't enumerated it). |
...
- **Bucket F** (subset): `tests/test_shared_templates.py` extensions (4 new assertions: impl-log instruction, parameterized byte-identity over PLAN_FILE, step-9 mandate wording, `/simplify` gating wording is in 5c — see below). `tests/test_makefile_review_targets.py` extensions: argv-capture test for backticks/`$(`-free prompt + Make-runtime PLAN_FILE passthrough test (set + unset for both codex + claude targets, the P2 impl-deferral item from iter-5).
...
2. `make status` runs in skill repo + outputs all 6 sections.
...
| 6 (codex) | 4 (2× imp-3, 1× imp-2, 1× imp-1) | needs decision — all 4 folded. **imp-3 trajectory now: 4 → 0 → 2 → 1 → 2 → 2** (3-iter plateau persists). Both imp-3 are real: P1 (current branch missing from status — feature-branch unmerged work invisible to recovery) is a real design gap; P2 (PLAN_FILE binding incomplete in subagent path) is a real coverage gap. Both folded properly: P1 via new "Current branch activity" section, P2 via macro signature update + subagent template binding. P3 + P4 trivial test/wording. **Decision point for user**: trajectory shows 6 iters with substantive findings each iter; this plan's scope (9 buckets) may exceed what one PR converges to cleanly. Options surfaced (2026-05-18): (a) iter-7 + maybe iter-8; (b) ship now with iter-7 findings as documented impl-PR folds; (c) split into 2-3 smaller PRs |
...
| 3 | 2 | **(a) fold** — Codex iter-3 #3: `tests/test_status_target.py` assertions only checked section headings → empty-section bug would pass while defeating the recovery purpose | Test row extended with 5 content assertions: known iteration-log row content, known impl-log row content, known LESSONS active title, README exclusion from plan-detect, bad-PLAN_FILE graceful handling |
| 3 | 2 | **(a) fold** — Codex iter-3 #4: Tier-1 `review-commit-by-*` recipes don't take `PLAN_FILE=`, but the prompt asks reviewers to check "plan-impl drift" — without a plan reference, the reviewer picks an unspecified plan (mtime drift again) | `tier1_prompt` macro extended to take an optional plan-file argument; Makefile recipes pass it through. CONTRIBUTING.md step 9 says to invoke `make review-commit-by-claude PLAN_FILE=docs/plans/<active>.md`. When unset, prompt falls back to "no specific plan; check against general project conventions" |
...
(Empty until impl PR starts — Tier-1 will propose entries as commits land.)
