...
3. **In-session ad-hoc subagent** (no make, no CONTRIBUTING template): if no PLAN_FILE provided, the rendered prompt's fallback text says the canonical phrase below (closes Codex iter-6 #2 — earlier wording allowed silent unbound plan-drift review).
