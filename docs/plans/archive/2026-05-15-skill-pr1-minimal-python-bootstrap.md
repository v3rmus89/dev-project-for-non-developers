...
> Per-PR expansion of subsystems A–F from the merged skill plan (`Acme docs/plans/2026-05-15-dev-project-setup-skill.md` (private repo)). This plan is concrete file-and-test detail; architectural decisions already settled in the merged plan are referenced, not re-litigated.
...
PR #1 is also the **bootstrap exception**: before this plan was added, the new repo contained only `README.md` + `.gitignore` (this plan file is the first substantive content; it lands together with the code per the bootstrap-exception pattern). The plan-review loop, the `make check` gate, and the AI-reviewer workflows are PART of what PR #1 ships, so they cannot gate PR #1 itself. PR #1's own plan is reviewed using Acme's existing `make review-plan` (Codex direction only). From Skill PR #2 onward the loop runs against the skill's own self-hosted bidirectional targets.

PR #1 lands plan + code in a **single PR** (the bootstrap-exception pattern explicitly documented in `docs/plans/README.md` of Acme and the merged plan's Build sequence).
...
- `requirements-dev.txt.tmpl` — pinned `ruff==0.15.12`, `pre-commit>=3.7,<5`, `pytest>=8.0,<9` (concrete versions match Acme's `~/code/acme/Telegram bot/requirements-dev.txt` — same toolchain on every generated project; closes Codex iter-6 finding #4)
...
`shared/` (12 files):
- `AGENTS.md.tmpl` — derived from Acme's `AGENTS.md`, retains "Plan Review Guidance" + per-language Codex review guidance, strips Acme-specific bot/API sections
- `CLAUDE.md.tmpl` — derived from Acme's `CLAUDE.md`, retains Commands table + Plan review loop + focused-commits guidance, strips bot-specific restart rule + API quirks + PINFL section
- `CONTRIBUTING.md.tmpl` — derived from Acme's `CONTRIBUTING.md`, retains per-change workflow + one-time setup + Codex CLI + Claude CLI install/login
- `BACKLOG.md.tmpl` — derived from Acme's `BACKLOG.md`; ships pre-populated with `--ephemeral` (Codex) and `--no-session-persistence` (Claude) parked entries, plus a "Claude/Codex CLI flag drift" entry whose trigger is `make preflight-review-tooling` failing, plus a "Retroactively add triage rule to Acme's plan-review docs" entry (trigger: after PR #1 lands)
- `pull_request_template.md.tmpl` — derived from Acme's `.github/pull_request_template.md`; reviewer checklist sections are `{% if %}`-gated by `--github-review` mode
- `claude-review.yml.tmpl` — derived from Acme's `.github/workflows/claude-review.yml`
- `docs-plans-README.md.tmpl` → renders to `docs/plans/README.md`; same workflow as Acme's plus the bidirectional review filename convention (`-by-codex-` / `-by-claude-`) and the bootstrap-exception clause
...
- `docs/plans/2026-05-15-skill-pr1-minimal-python-bootstrap.md` — **this plan file itself** (Codex iter-4 finding #5): PR #1 lands plan + code together (bootstrap-exception pattern documented at the top of this file). The plan remains committed permanently in `docs/plans/` after PR #1 ships — it's the institutional record of why these design decisions were made (per Acme's `docs/plans/README.md` rationale: "Plans become institutional memory").
...
**Dogfood-docs Acme-isms scan (Codex iter-10 finding #3)**: even though the overlay docs aren't byte-for-byte selftested, PR #1 ships `tests/test_dogfood_doc_sanity.py` that runs the same no-Acme-isms scan from Subsystem C tests against the committed `AGENTS.md`, `CLAUDE.md`, `CONTRIBUTING.md`, and `.github/pull_request_template.md` at the skill repo root. Forbidden terms: `Telegram`, `bot/`, `PINFL`, `customs`, `signup`, `payment`, `i18n/ru.json`, plus the origin project's real name and `<name>.db`. NOTE: the denylist literals in `tests/test_dogfood_doc_sanity.py` and `tests/test_shared_templates.py` deliberately keep the REAL pre-scrub terms rather than the `Acme` placeholder — those strings are the detector, and genericizing them would leave both guards green while detecting nothing. Allowlist: this plan file (`docs/plans/2026-05-15-skill-pr1-minimal-python-bootstrap.md`) AND any file under `docs/plans/` is allowed to reference Acme because the plans cite Acme as the source-of-truth for the patterns — those references are historical attribution, not active guidance.
...
- **Python**: 3.12+ (matches Acme + supports `str | None` union syntax)
...
**Source-of-truth mapping** (each .tmpl derives from a Acme file):

| Template | Acme source | Notes on what to strip |
|---|---|---|
| `shared/AGENTS.md.tmpl` | `~/code/acme/Telegram bot/AGENTS.md` | Strip Telegram-bot-specific review guidance; retain Plan Review Guidance + per-language Codex review guidance template. **Includes the "Don't fold by default — triage" rule** (see "Triage rule for both reviewers" below) — applies when Codex reviews a Claude-authored plan |
| `shared/CLAUDE.md.tmpl` | `~/code/acme/Telegram bot/CLAUDE.md` | Strip bot-restart rule, API quirks, PINFL section, City/district logic; retain Commands table, Plan review loop, focused-commits guidance. **Includes the "Don't fold by default — triage" rule** — applies when Claude reviews a Codex-authored plan |
| `shared/CONTRIBUTING.md.tmpl` | `~/code/acme/Telegram bot/CONTRIBUTING.md` | Retain per-change workflow, one-time setup, Codex CLI install/login, Claude CLI install/login |
| `shared/BACKLOG.md.tmpl` | `~/code/acme/Telegram bot/BACKLOG.md` | Strip Acme-specific entries; ship with four starter entries: `--ephemeral` (Codex), `--no-session-persistence` (Claude — symmetric to `--ephemeral`), "Claude/Codex CLI flag drift" (trigger: preflight target fails), "Retroactively add triage rule to Acme's plan-review docs" (trigger: after PR #1 lands) |
| `shared/pull_request_template.md.tmpl` | `~/code/acme/Telegram bot/.github/pull_request_template.md` | Reviewer-checklist section wrapped in `{% if github_review_mode in ['claude','both-docs'] %}` for claude[bot]; `{% if github_review_mode == 'both-docs' %}` for chatgpt-codex-connector[bot] |
| `shared/claude-review.yml.tmpl` | `~/code/acme/Telegram bot/.github/workflows/claude-review.yml` | Substitute `{{github_owner}}` / `{{github_repo}}` for any repo-pinned references. **Generic-prompt rewrite (Codex iter-4 finding #3)**: Acme's workflow prompt explicitly mentions Telegram bot, payments, API drift, i18n, PII, PINFL — all bot-specific. Rewrite the prompt to be project-agnostic: instruct Claude to read the generated `AGENTS.md` + `CLAUDE.md` for project-specific risk classes, and review against THOSE rather than against any baked-in domain. Generic risk categories to keep: security vulnerabilities (OWASP-style), regressions, hard-coded secrets, test coverage gaps. |
| `shared/docs-plans-README.md.tmpl` | `~/code/acme/Telegram bot/docs/plans/README.md` | Add bidirectional-review filename convention paragraph; add bootstrap-exception clause referencing the skill's bootstrap-exception (PR #1). **Documents the "Don't fold by default — triage" rule** as part of step-2 of the workflow (see "Triage rule for both reviewers" below) — this is what makes the loop terminate faster (the merged Acme plan converged in 3 iterations; this skill plan took 16 because every finding was folded by default) |
...
| `shared/Makefile.review.tmpl` | derived from `~/code/acme/Telegram bot/Makefile` lines 66–115 | Three targets (codex, claude, preflight); reviewer-aware `PLAN_REVIEW_OUT` filename; `env -u` scrub list enumerated explicitly |
| `shared/scripts-run-with-clean-env.py.tmpl` | new (no Acme source) | Same content as the skill repo's `scripts/run-with-clean-env.py`; emits to `scripts/run-with-clean-env.py` in generated project, mode 0755. Listed in Subsystem D as the actual review-tooling dependency, repeated here for completeness so Subsystem C's mapping table is exhaustive (closes Codex iter-2 finding #5). |

### Triage rule for both reviewers (the "Don't fold by default" rule)

Lessons-learned from this plan's 16-iteration convergence (vs the merged Acme plan's 3): the loop terminates faster when the plan author triages findings instead of folding everything. This rule is templatised into `shared/CLAUDE.md.tmpl`, `shared/AGENTS.md.tmpl`, and `shared/docs-plans-README.md.tmpl` so every bootstrapped project inherits it. It's ALSO retroactively added to the skill repo's own Bucket-3 hand-written `CLAUDE.md`/`AGENTS.md`/`docs/plans/README.md` (so the skill repo itself uses the rule for PR #2 onward).
...
**Out of scope for PR #1**: applying this same rule retroactively to Acme's `CLAUDE.md` + `AGENTS.md` + `docs/plans/README.md`. That's a small Acme-side PR that lands separately — flagged in BACKLOG as a follow-up. Reason it's not in PR #1: PR #1 ships only to this new repo; touching Acme would expand the PR scope across two repos.
...
- **No-Acme-isms scan (Codex iter-4 finding #3)**: per-template rendered-content scan asserts NONE of the following strings appear in any shared template's output: `Acme`, `Telegram`, `bot/`, `PINFL`, `i18n/ru.json`, `acme.db`, `customs`, `signup`, `payment` (case-insensitive). This catches accidental leaks from copying Acme's domain-specific text into a generic template. If a template legitimately needs to reference the project (e.g. `{{project_name}}` placeholder), it uses a Jinja variable, not a hard-coded string.
...
review-plan-by-codex:
    ...
    $(CURDIR)/scripts/run-with-clean-env.py \
      -- codex exec -C "$(CURDIR)" --sandbox read-only --color never \
         ...
         "<review prompt — same as Acme's, modulo iteration text>"
...
The skill repo's `Makefile` matches Acme's pattern (closes Codex iter-5 finding #3 — bare `pip`/`pytest`/`ruff`/`pre-commit` would use whatever's on global PATH, defeating the "same gate" claim and tripping on the system `python3` being 3.9.6 vs `python3.12` available separately):
...
The skill repo's `requirements-dev.txt` (concrete pins per Codex iter-6 finding #4 — matches Acme where applicable, deliberate choice elsewhere):
...
**Acceptance gate**: `cd ~/code/dev-project-for-non-developers && make check` green locally; CI green on the PR.

## Risks + mitigations (PR-#1-specific)

| Risk | Mitigation |
|---|---|
| PR #1 doesn't have CI on its first push (workflow file is part of what it ships) | Expected and documented. `claude[bot]` review may skip on PR #1 itself — the third+-doc-only-PR pattern from Acme. Codex auto-review may also skip. Reviewers run locally via `make check` + smoke walk + plan-review loop. From PR #2 onward CI runs on every push. |
...
1. **Plan-review loop** run on THIS plan via Acme's `make review-plan` (the bootstrap exception — this repo doesn't self-host the loop yet). Stopping rule per merged plan / `docs/plans/README.md`: triage rule applied; remaining imp-3 findings either folded or explicitly accepted-as-trade-off and surfaced to human approval.
...
| 24 | 4 (Codex re-review on the iter-23 fold commit; 3× P1 + 1× P2) | mixed triage — 3 of 4 findings are stale re-flags Codex has now repeated across iter-23 and iter-24 at the exact same line numbers despite the fixes being in place: P1 manifest.py:186 (write-back uses `bio.atomic_write` at line 185); P1 cli.py:279 (manifest path + rollback hint print at lines 272-278); P2 io.py:52 (unlink in `finally` at lines 53-55). All three REJECTED again. 1 new real finding: P1 cli.py:178 "mkdir before manifest write" — `_prepare_apply` called `root.mkdir(parents=True)` before `manifest.write_manifest`, so if the process died between, `--out` was created with no rollback manifest. FOLDED by removing the pre-manifest mkdir; `atomic_write` already creates parent dirs per file, so target_root is created lazily on first write — AFTER the manifest is durable. Test `test_target_root_not_created_when_manifest_write_fails` monkey-patches `manifest.write_manifest` to raise; asserts target dir not on disk afterward. |
...
| 4 | 3 | `claude-review.yml.tmpl` derived verbatim from Acme's workflow whose prompt references Telegram bot, payments, API drift, PINFL — domain leakage into every generated project's PR reviewer | Subsystem C mapping table now requires generic-prompt rewrite: Claude reads generated `AGENTS.md`/`CLAUDE.md` for project risks rather than baked-in domain. Added no-Acme-isms scan to `tests/test_shared_templates.py` (fails on `Acme`, `Telegram`, `bot/`, `PINFL`, `customs`, `signup`, `payment`, etc. case-insensitive) |
...
| 10 | 2 | Dogfood `AGENTS.md`/`CLAUDE.md`/`CONTRIBUTING.md` were excluded from selftest because they have overlays, but the no-Acme-isms scan only ran against shared templates — overlay docs could still leak Telegram/payment/PINFL guidance and steer PR #2's self-hosted reviews wrongly | Added `tests/test_dogfood_doc_sanity.py`: same forbidden-terms scan as Subsystem C, applied to committed dogfood `AGENTS.md`/`CLAUDE.md`/`CONTRIBUTING.md`/pull_request_template.md. Allowlist for the plan files under `docs/plans/` (historical attribution to Acme is intentional there) |
...
| 18 | 2 | **(a) fold** — `restart` listed in generated Python Makefile surface but had no spec or smoke coverage; it's a Acme-ism (bot restart) that would always ship because PR #1 always emits `src/main.py`, contradicting the "only when run-able" hedge | Removed `restart` from `languages/python/Makefile.tmpl` row. Surface is now `help / install / install-hooks / test / lint / format / check / run`. No spec or test for restart in PR #1 |
...
| 24 | 3 | **(c) reject ×3** — Codex re-review on iter-23 commit repeated 3 already-folded findings (P1 manifest.py:186, P1 cli.py:279, P2 io.py:52) at unchanged line numbers despite the fixes being verifiably in place. Pattern across iter-23 → iter-24 suggests Codex's GitHub auto-review is running against a cached snapshot or its analysis is not refreshing on `@codex review` triggers within the same PR | No code change. Evidence row documents the repeated rejection so future reviewers see the pattern. The three findings remain valid as historical bugs (now closed); Codex's persistence on them is a tooling artefact, not new information |
| 24 | 2 | **(a) fold** — Codex re-review on iter-23 commit (P1, new): `_prepare_apply` did `root.mkdir(parents=True)` BEFORE `manifest.write_manifest`, so if the process died between the mkdir and manifest fsync the user had a partial side effect (target dir created) with no rollback path. Violates the spirit of "no fs mutation before the manifest is durable" even though the documented contract is specifically about file `os.replace` calls | Removed the pre-manifest `mkdir`; `atomic_write`'s per-file `target.parent.mkdir(parents=True, exist_ok=True)` already creates the tree lazily on the first write. Target_root is now created AFTER the manifest is fsync'd. `plan_entries` already handles non-existent root correctly. Test `test_target_root_not_created_when_manifest_write_fails` monkey-patches `manifest.write_manifest` to raise; asserts target dir not on disk |
...
- **No retroactive update of Acme's `CLAUDE.md`/`AGENTS.md`/`docs/plans/README.md`** to add the triage rule. That's a separate Acme PR (touching another repo would expand PR #1's scope across two repos). Tracked as a BACKLOG follow-up; trigger: after PR #1 lands and we run the next substantive Acme plan-review.
...
- `~/code/acme/Telegram bot/docs/plans/2026-05-15-dev-project-setup-skill.md` — the merged plan; subsystems A–F are defined there
- `~/code/acme/Telegram bot/Makefile` lines 66–115 — source of `Makefile.review.tmpl`'s `review-plan-by-codex` target
- `~/code/acme/Telegram bot/.github/workflows/claude-review.yml` — source of `shared/claude-review.yml.tmpl`
- `~/code/acme/Telegram bot/.github/pull_request_template.md` — source of `shared/pull_request_template.md.tmpl`
- `~/code/acme/Telegram bot/CLAUDE.md`, `AGENTS.md`, `CONTRIBUTING.md`, `BACKLOG.md`, `docs/plans/README.md` — sources of the corresponding `shared/*.tmpl` files
- `~/code/dev-project-for-non-developers/README.md` — the new repo's framing (repo name vs skill name distinction)
- `~/code/dev-project-for-non-developers/.gitignore` — the new repo's only other existing file
