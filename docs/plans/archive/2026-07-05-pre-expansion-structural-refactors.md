...
- Retarget the shared-template SCAN tests (Tier-2 codex P1, verified): the six
  ...
  `test_no_private_domain_terms_in_shared_templates` call `get_template()` on each — after
  ...
  Jinja templates, so "renders without undefined" and "no private-domain-terms in a
...
| (fold 2) | Tier-2 cross-AI review folds — local codex stand-in on gpt-5.6-sol (GitHub Codex connector down: OpenAI backend 5xx outage; claude[bot] blocked pending PR #51's model-pin fix). 0 imp-3 / 3 imp-2, verdict needs-another-iteration; all 3 folded (a). FN1: verbatim sources now pass `validate_target_path` before reading (absolute / `..` / symlink escape rejected — codex empirically shipped `/etc/hosts` through the unguarded join) + 2 negative tests in `tests/test_render.py`. FN2: verbatim-source genericity scan restored (`test_no_private_domain_terms_in_verbatim_sources`, parametrized over the map so Bucket B prompts inherit it) — the plan's "byte-equality is the replacement coverage" under-enumerated the retired scan's properties; deliberate beyond-plan strengthen. FN3: inventory claims precisified in `docs/usage.md` + `render.py` comment (adopt's post-`render_all` standalone `Makefile.review` injection, `.new` companions, `--dry-run` scope) | — | New LESSONS.md entry: replacement-coverage property enumeration |

Bucket B (branch `refactor/bucket-b-prompt-extraction`):

| Commit | Change | Tier-1 | Notes |
|---|---|---|---|
| `cda3b51` | Five review prompts extracted to `prompts/*.txt` (verbatim + registry tokens; `{VERIFICATION_JSON}` file-backed), built at runtime by `scripts/render-review-prompt.py` (known-token registry, single-pass, env `NAME` else `NAME_FILE`, exit-2 fail-loud; dual exec-bit); all 9 recipe call sites `PROMPT="$$(VARS… helper file)" \|\| exit $$?` with vars inside the substitution; `tier1_prompt` macro retired (template now byte-equals the Makefile review section — zero Jinja); CONTRIBUTING (tmpl + dogfood) documents both subagent variants via the prompt files; ab-replay hand-mirror dropped (`ab_replay_lib` reads the shared file, computes `{KEY}` with the Makefile formula); `/simplify` merged into the Tier-1 focus lists; `migrate-selftest-block.py` carries prompts+helper and gains its FIRST tests (dry-run diff, `--apply` 0755, `REVIEW_RESOLVE=1` end-to-end) | 0 imp-3 / 1 imp-2 / 0 imp-1 | Retired-test properties each mapped to a surviving owner (macro byte-identity → single-source + CONTRIBUTING-references test; empty-string≡None → recipe `[ -n ]` branch via argv shim tests; no-backticks → all-five-files shell-safety test). Plan's "update adopt planned-set fixtures" was a verified no-op: fixtures derive counts dynamically from `render_all` — no test pins an absolute planned-set count. `make check` 1098 passed / 4 skipped (net +70); `make preflight-review-tooling` ok; one real `make review-plan-fact-check-by-codex` against this plan through the new helper path (no imp-3, failures all benign proposed-Bucket-C/D/E files) |
| `01cfdf6` | Tier-1 fold: F1 (a) — Active `LESSONS.md` 2026-05-19 rule still pointed at the retired `tier1_prompt` macro; retargeted to the `prompts/commit-review-*.txt` files + substitution sets (the 2026-05-18 lesson's Trigger mention stays: past-tense historical record) | — | New LESSONS.md entry: deleting a named construct must sweep ALL live guidance docs for forward-pointing references — the plan's enumerated surface list is a floor, not the sweep |
| (fold 2) | Tier-2 cross-AI folds — local codex stand-in (GitHub connector silent >6 min on `@codex review`, consistent with #50/#51; claude[bot] + CI blocked by a GitHub Actions **billing failure** on the account, jobs never started). 1 P1 / 2 P2, verdict needs-changes; all 3 folded (a), premises verified. P1: `migrate-selftest-block.py` derived destinations get `validate_target_path` containment (a symlinked `prompts/` dir or helper file pointing outside the project would let `--apply` — and dry-run's read — escape; both modes now exit 2 `refusing`) + writes go through same-dir temp file + `os.replace` (never follows a dst symlink; crash-atomic) + 2 symlink-escape tests. P2: byte-identical helper with a lost exec bit now counts as drift (`(mode)` status; dry-run names it, `--apply` chmods 0755) + mode-only fixture test. P2: ambient exported `VERIFICATION_JSON` would shadow the file-backed verifier output (helper prefers env NAME by contract; the subshell inherits the outer env) — both fact-check recipes pin `VERIFICATION_JSON=""` inside the substitution (empty = unset → falls through to `_FILE`) + shadow test for BOTH actors | — | Codex verified ruff + 62 targeted tests in its sandbox; full suite blocked there (no writable tmpdir) — local `make check` is the gate |

Bucket C (branch `refactor/bucket-c-cli-split`):

| Commit | Change | Tier-1 | Notes |
|---|---|---|---|
| `bf35dbe` | `cli.py` 1283 → **409 lines**; 23 names moved verbatim into `bootstrap_lib/adopt_ui.py` (216), `bootstrap_lib/apply_pipeline.py` (422), `bootstrap_lib/guidance.py` (270). `SKILL_ROOT`/`BOOTSTRAP_PY` follow their sole consumer `_format_restore_hint` into guidance (same nesting depth → value unchanged). The two lazy `bootstrap_lib.adopt` imports become top-of-module imports; BOTH binding forms kept (module + two-name from-import) so the bodies stay byte-identical instead of gaining `adopt.` prefixes. No re-exports (AD4); test imports updated same commit | 0 imp-3 / 0 imp-2 / 1 imp-1 | Layered graph verified by AST walk — `cli → {apply_pipeline, guidance}`, `apply_pipeline → {adopt_ui, guidance}`, both leaves stdlib-only, nothing imports `cli`. `cli` omits the plan's allowed `adopt_ui` edge: it needs no name from it and an unused import is F401 — a subset of the allowed edges, not a violation. Pure-movement proven by an AST checker comparing all 35 top-level names between HEAD's `cli.py` and the new modules (byte-identical, no re-exports, nothing unclassified) — name+span based, so it cannot pass by construction. `make check` 1222 passed / 4 skipped, identical to main (a pure move adds no tests). Beyond the suite: a greenfield `--dry-run` and an interactive `--mode=adopt --apply` against a Makefile-owning target, which crosses every new module edge and exits 0. `main()` moved verbatim, NOT restructured — the plan's "dispatch table" phrasing describes its shape, and rewriting it is what AD6 keeps out of a movement commit |
| `3ad85f0` | Tier-1 fold: imp-1 (a) — prose pointers still naming `cli.py` as the home of moved code. Reviewer named 4 (adopt.py + 3 test docstrings); the live-guidance sweep found **13 across 9 files** — also `manifest.py`, `render.py`, `CLAUDE.md`'s path-safety invariant, `docs/usage.md`, one still-parked `BACKLOG.md` item, `test_path_safety.py` | — | Module paths only; no rewording, no de-citation (Bucket E). LESSONS.md 2026-07-26 confirmed again: the reviewer's list is a floor, not the sweep. Deliberately untouched: BACKLOG's two ✅ closed entries citing `cli.py:589`, `adopt.py`'s "per Bucket A `cli.py` row" (plan-table citation, Bucket E's de-citation rule owns it), and `docs/plans/*` citations (point-in-time records — same principle as the closed BACKLOG entries) |
| `ff1294d` | Tier-1 fold: imp-2 (a) — the retargeted path-safety pointer named the DEFINITION site (`apply_pipeline.py:35`) and dropped the CALL site (`cli.py:347`, inside `main()`, guarding all three modes). For an invariant whose job is "both must remain in place", that is backwards: the call could be deleted, the function left defined, and the invariant read as satisfied. All three sites in the class now name function + caller | — | Premise verified against both files before folding. Swept the other 6 retargeted pointers for the same definition-vs-call confusion — each already names a caller, so all were correct as written. **Parked to Bucket E** (already-scheduled owner, no BACKLOG entry needed): Tier-1's second imp-1 — `adopt.py:362` / `manifest.py:357` say `AdoptionCollisionError` is "converted to `CLIError(exit_code=2)`" when the code writes to stderr and returns 2. Pre-existing inaccuracy, carried through untouched; it is rewording, not a pointer fix |

Bucket D (branch `refactor/bucket-d-process-weight`):

| Commit | Change | Tier-1 | Notes |
|---|---|---|---|
| `f135fb5` | 18 merged plans → `docs/plans/archive/` (13 pure renames + 5 rename-with-content-change); 24 repo-local references swept across the 9 files the plan enumerated (verified independently — the 12-file surface reconciles as 9 touched + 3 deliberately untouched); 31 link targets rewritten in 5 moved plans (`../`-forms one level deeper, `../../`→`../../../`, and the 8 bare-filename links already broken BEFORE this PR normalized rather than allowlisted); archive convention added to the byte-locked `docs/plans/README.md` + `shared/docs-plans-README.md.tmpl` pair | 0 imp-3 / 0 imp-2 / 2 imp-1 | `make check` 1222/4 — identical to main (a move + sweep adds no tests). `make status`: single active plan, multi-plan WARN gone. **Deliberately NOT swept**: the 44 backticked `docs/plans/…` citations inside plan files — inspection showed they are `make review-plan-by-codex PLAN_FILE=…` command transcripts, self-references, sibling-repo paths, and references to throwaway files that never existed here; rewriting them would corrupt a historical record rather than fix a pointer |
| `dc51637` | `tests/test_markdown_link_integrity.py` — general in-repo link gate over every tracked `.md`, skipping inline-code spans + fenced blocks, reusing `scripts/extract-plan-facts.py::_FenceTracker` rather than a fourth hand-rolled toggle; carries its own anti-vacuity checks (populated corpus, minimum link count). `DEFAULT_PLAN` existence assertions in `tests/test_ab_replay_lib.py` + `tests/test_verify_v13_5.py` (NOT the iter-0 `--help` smoke — iter-1 FN3) | 0 imp-3 / 0 imp-2 / 3 imp-1 | Regression-proven against unfixed states in a throwaway worktree BEFORE trusting it: 1 failure on main as-is (the 8 pre-existing breaks), **8** failures on main-plus-the-move-without-rewrites (every break class), and both `DEFAULT_PLAN` assertions fail when the constants are reverted. `make check` 1271/4 |
| `0bfafee` | `BACKLOG.md` split: 21 closed entries → `BACKLOG-archive.md`, 46 active stay (the 🟡 partial counts as open — its non-interactive half is still parked); section structure mirrored, intro prose kept with the active side unless the whole section moved. Two new entries: the review-loop A/B-win standing policy (from the NOT-in-scope table) and the `--mode=upgrade` ownership-model engine with its trigger. `shared/BACKLOG.md.tmpl` untouched | 0 imp-3 / 0 imp-2 / 2 imp-1 | Split verified structurally, not by eye: all 67 headings appear exactly once across the two files, none in both, every entry body byte-identical to its pre-split form. `test_backlog_has_deferred_bucket_entries` survives — both required slugs stay in `BACKLOG.md`, F-series refs 24 ≥ 4. `make check` 1275/4 |
| `d295be6` | `docs/usage.md` "Bringing an adopted project up to date" — three ordered sync steps (`propagate-shared-rules.py` → `migrate-selftest-block.py` → adopt re-run), each read-only first, with the in-section-owner-edit caveat stated plainly and the `--diff`-not-`--dry-run` adopt nuance called out | 0 imp-3 / 0 imp-2 / 2 imp-1 | Every flag verified against each script's actual `--help` rather than recalled. `make check` 1275/4. Each of the four commits was additionally verified individually green in a worktree — not just the tip |
| `b5598b5` | Tier-1 folds (all 4 commits reviewed; the target hardcodes `COMMIT_REF=HEAD`, so the same prompt file was driven for the other three refs). 0 imp-3 / 0 imp-2 / 9 imp-1: **6 folded (a)** — link gate widened to reference-style definitions + angle-bracket targets with 6 new tests (the reviewer offered the cheaper "document the hole"; documenting a gap in a gate whose promise is "rot cannot happen silently" is the wrong trade), `git ls-files -z`, fence-boundary edge documented, A/B figure precisified to the price-independent uncached ratio, runbook interpreter unified; **1 parked (b)** — `markdown-anchor-resolution` (anchors need GitHub's slug algorithm incl. duplicate suffixes; a gate that false-fails gets deleted); **2 rejected (c)** — the "illustrative link syntax" is a backticked `](path)` fragment that cannot match the regex, and the 1371-vs-1347 line count is the file growing between plan and implementation | — | `make check` 1281/4. Commit `f135fb5`'s body originally said 6 files of link rewrites; true count is 5, corrected in place while unpushed (resulting tree byte-identical, verified by empty diff). New LESSONS.md entry: counts in durable records must be derived from the artifact, not from the enumeration that preceded the work |

| (Tier-2 fold) | claude[bot] Tier-2 on PR #58: **0 imp-3 / 0 imp-2 / 0 findings — "nothing to fold"**; it independently re-verified the README byte-lock, the dogfood BACKLOG test, the sweep being pointer-only, and both Tier-1 widenings as genuine correctness fixes. One INFORMATIONAL note folded anyway (a): `broken_links` joined a root-relative target (`/docs/foo.md`) onto the linking file's directory, so it resolved against the FILESYSTEM root. The bot declined to call it a finding because it fails closed — but failing closed here means a false FAILURE on valid GitHub markdown, and "a gate that false-fails gets weakened or deleted" is the exact argument used to park anchor resolution. Root-relative targets now resolve against the repo root, with a positive and a negative test. Premise verified first: `Path('docs/plans') / '/docs/foo.md'` → `/docs/foo.md`, and no live instance exists in the corpus | — | `make check` 1283/4. CI green (`check` + `claude-review`) |

| (Tier-2 fold 2) | codex connector on PR #58: **7 P2, 0 P1** — 6 folded (a), 1 parked (b); every premise verified against the cited file first. **FN6 was a real bug in a SHIPPED template**: `git mv X docs/plans/archive/` fails `fatal: destination directory does not exist` in a fresh checkout (git tracks no empty dirs) — reproduced in a scratch repo; both byte-locked copies now carry `mkdir -p`. I had run that exact `mkdir` myself during implementation and failed to put it in the convention I documented. FN7: one entry (`review-plan-fact-check-by-{claude,codex}`) whose heading lacked ✅ but whose Status said "shipped in PR #30" stayed active — my splitter classified on the heading alone; audited all 49 active entries for body-level shipped status and this was the ONLY instance. FN5: the A/B policy named `scripts/ab-replay.py` as the harness, but it compares fresh-vs-continued sessions against ONE repo state — it cannot compare feature variants; entry rewritten to describe the two-checkout procedure and to claim ab-replay only as precedent + plumbing. FN1: the step-2 dry run prints a full diff for the Makefile block but only a `(changed)` LABEL for the carried `prompts/*.txt` + helper, so "read the diff first" overstated its guard — runbook now says so and tells the reader to commit/stash before applying to a customised project. FN2 + claude[bot]'s note: externality now decided by URI SCHEME, not by `://` — `tel:`/`sms:`/`data:`/`//host` were all being resolved as repo paths. FN4: containment enforced — `../../etc/passwd` existed locally and PASSED, a silent false pass on a link that 404s for every reader. FN3 parked (b): balanced-paren destinations — zero instances (plan files are dated slugs) and the angle-bracket form already works, so the limitation is documented with its workaround rather than parsed | — | `make check` 1287/4. The containment fold broke 3 of my own tmp_path tests (they used the default root) — caught by the suite, fixed by passing `root=tmp_path` |

| (Tier-2 fold 3) | codex re-review of the folds: **4 P2, 0 P1 — all folded (a)**. Two are silent-PASS classes, the direction that matters: a link wrapped in BACKSLASH-escaped backticks was masked as a code span though CommonMark leaves the link ACTIVE, and a target existing on disk but ABSENT from the git index passed locally while 404-ing for every reader after push (gate now takes the tracked set; 0 live instances, so the suite stayed green). Two are false-FAILURE classes: percent-encoded destinations (`a%20b.md` for `a b.md`) now decode before lookup. **GN2 the reviewer UNDERCOUNTED**: it said the runbook's `propagate-shared-rules.py` call should name 3 byte-locked triage surfaces; `rg -l '^## Triaging review findings'` shows **4** dogfood surfaces (it missed `CONTRIBUTING.md`), all 4 in `SHARED_TEMPLATE_MAP` — runbook now names all four. LESSONS 2026-07-26 again: the reviewer's list is a floor, not the sweep. **Codex loop STOPPED here** (plateau, per CONTRIBUTING's same-class-regeneration rule): 0 P1 across both rounds, and round 2 produced only link-parser edge cases with zero live instances in the corpus — the artifact is now verified by execution (65 tests on the gate itself) rather than by another review round | — | `make check` 1292/4 |

| (CI fix) | **CI caught what my local run could not.** The escaped-backtick fold changed the meaning of a form used in the previous row's own prose, turning an illustrative example into a live link to a nonexistent file — and I had run `make check` BEFORE appending that row, so the committed tree was never the verified tree. Row de-linked. Also folded claude[bot]'s last informational note (a): `_is_tracked` computed `"."` for the repo root, which is never a key in the tracked set, so a link to the repo root itself would have false-failed; handled with a test. **This is why the codex loop stopping was the right call and the CI gate was not**: two AI review rounds passed this commit, and an actual execution caught it | — | `make check` 1293/4, run AFTER the final edit. New LESSONS.md entry: verification must cover the FINAL tree — a doc edit is input to any gate that reads docs |

Bucket E (branch `refactor/bucket-e-comment-naming-hygiene`):

| Commit | Change | Tier-1 | Notes |
|---|---|---|---|
| `c2ad259` | De-citation pass over `bootstrap_lib/` (12 files) + `scripts/` (4 files), +178/−182 lines: review-tracker citations dropped with each constraint kept; citation-only tails deleted; plan-ID contract names reworded plain ("Scope #11 privacy boundary" → "the privacy boundary", "Scope #5 rules" → "the adoption rules (a0/a..h)"); durable-doc pointers kept (`LESSONS.md`, `CONTRIBUTING.md`, `adopt.py`'s archived-plan path — the anchor for the contract vocabulary). Pinned-surface pre-check first: no test asserts on `bootstrap_lib/`/`scripts/` comment or docstring text; `scripts/render-review-prompt.py` prints its docstring at runtime and was left untouched. Runtime strings untouched (`KEEP_V13_5_JSONL`, `[V-13.5]` tags, `help=`, probe paths) | 0 imp-3 / 0 imp-2 / 2 imp-1 — both reviewer-marked no-change, (c): the "V-4 → manual env preflight" rewording was additionally verified against the archived A/B plan's own V-4 definition; the `analyze_target` "by design" wording carries its constraint inline | Executable AST proven identical to HEAD per file (docstrings stripped, `ast.dump` compared — the same normalizer for all five commits). `make check` 1293/4 on exactly this tree |
| `4b4f203` | The Bucket C parked item (`ff1294d` row): `AdoptionCollisionError` docstrings in `adopt.py` + `manifest.py` claimed conversion to `CLIError(exit_code=2)`; both catch sites in `_main_apply_adopt` write stderr + return 2 — reworded to reality, and the single-trigger claim extended to both trigger classes (`.new` collision + the NEUTRALIZE TOCTOU guards). Sweep for a third stale conversion claim: none (`apply_pipeline.py:110` already accurate) | 0 imp-3 / 0 imp-2 / 1 imp-1 (a) — "appears before apply" wider than the checked window; folded in `024cda9` as "plan→apply window" | Docstring-only; AST-identical. `make check` 1293/4 on this tree |
| `24f6835` | `scripts/verify-v13-5.py` → `scripts/verify-codex-thread-continuation.py` + `tests/test_verify_v13_5.py` → `tests/test_verify_codex_thread_continuation.py` (git similarity 98%/97%, exec bit 100755 preserved); references updated: test loader path + importlib spec name + one assert message (the only string-constant AST deltas, enumerated), `scripts/ab-replay.py` docstring, `BACKLOG.md`'s active V-13.5 entry, `tests/test_markdown_link_integrity.py` docstring. Variant sweep per LESSONS 2026-07-27: kebab/snake/case forms + scaffolding (Makefile, CI, configs — zero hits) | 0 imp-3 / 1 imp-2 / 1 imp-1. imp-2 (a, folded into the durable records rather than a history rewrite): the commit message's exclusion clause named "iteration and implementation logs" but the untouched set also includes the plan's scope row 4, Bucket D section body, and D's verification row — all point-in-time records of Bucket D's MERGED work, deliberately kept under the same discipline as the log rows (rewriting them would claim D verified a filename that did not exist at D time; message amendment would have rewritten two reviewed SHAs). imp-1 (a): reflow of three wrap artifacts, folded in `024cda9` | Renamed script AST-identical (docstring-only edits); runtime `[V-13.5]`/`KEEP_V13_5_JSONL`/probe names retained by design. `make check` 1293/4 on this tree |
| `c0a4e50` | `render_all`'s function-local `import tempfile` hoisted to `bootstrap_lib/render.py` module top — the PR's only intentional AST change, verified to be exactly one `Import(tempfile)` leaving the function body and one arriving at module level | 0 imp-3 / 0 imp-2 / 1 imp-1 — informational, reviewer-marked no-change (c) | Tree byte-identical to the pre-approval verified final state (5 dual-touched files `cmp`-checked). `make check` 1293/4 |
| `024cda9` | Tier-1 folds: the two (a) imp-1s above — TOCTOU-window wording + three docstring reflows | — | AST-identical ×4. `make check` 1293/4. Tier-1 ran per commit: HEAD via `make review-commit-by-claude`, the other three by driving `prompts/commit-review-plan-bound.txt` with `COMMIT_REF=<sha>` through the same clean-env pipeline (the recipe pins `COMMIT_REF=HEAD`) |

## Lessons surfaced (this PR)

- **2026-07-29** — a count taken from the categories reasoned about rather than the artifact produced (`f135fb5`'s "6 files" vs the script's 5). Added to `LESSONS.md` Active.
