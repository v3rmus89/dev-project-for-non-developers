...
| NEW: `tests/test_mode_adopt_smoke.py` | End-to-end `--mode=adopt --auto-accept-recommendations` against a synthetic fixture matching call-details collision shape (**4 collisions + 15 clean writes** — closes self-check iter-1.5 #2). Asserts: collision-abort does NOT fire; per-file policies match expected (4 collisions get SKIP/APPEND_MERGE/WRITE_NEW/SKIP per rules d/e/f/g; 15 clean writes get WRITE); restore manifest captures the policies; restore works (each policy type's restore semantics tested). |
...
|---|---|---|
| A (partially done during plan iter-1 fold; remainder runs after plan converges) | Run `bootstrap.py --dry-run --language python --project-name call-details --out ~/Desktop/Code/Boxette/call-details/` (PR #6 contract). **DONE during iter-1 fold** — result: 4 MODIFY + 15 CREATE, documented in Context "Empirical collision baseline" subsection. **REMAINING for post-converge**: run `--diff` (same flags) for each of the 4 collisions; inspect target's actual file contents per collision; capture file sizes, sha256 hashes, structural categories (NOT content excerpts — per privacy boundary in Scope #11). | Empirical findings dataset → `docs/trial-report-pr7.md` Section (a) + (b). Adjusts the heuristic rules in Bucket B's recommend_policy if Phase A surfaces a misfire. |
| D (post-impl) | **Preflight** (closes Codex iter-1 #4): capture branch/SHA + clean-status, create `feat/pr7-trial-adoption` branch in TARGET (not skill repo), record planned-files list. Run `bootstrap.py --apply --mode=adopt --language python --project-name call-details --out ~/Desktop/Code/Boxette/call-details/` for real. Owner reviews each interactive prompt. Post-apply: record `git diff --stat`, verify restore-manifest round-trip on a copy of the target. | Trial-report Section (d) — was call-details/ better off after? Anything broken? Any user-facing UX gaps? Branch SHAs + diff-stat summary (NO diff content per privacy boundary). |
