...
| 12 | Tests: (i) extend `tests/test_bootstrap_cli.py` with **7 NEW tests** total: 3 detection-state (no-`.git/`, `.git/`-no-remote, has-remote-no-fire), 1 regression guard (no literal `git add -A`/`.`), 1 visibility-placeholder guard (no hardcoded `--public`), 1 none-mode-no-hint guard, 1 both-docs-points-at-setup-doc check. (ii) Update `tests/test_dogfood_doc_sanity.py:125` invariant to allow `@codex review` in root CLAUDE.md (Scope item 4). (iii) `tests/test_selftest_overlap.py::test_overlap_docs_plans_readme` must continue to pass (Commit 1 impacts it). (iv) **Must-preserve-token discipline** (iter-3 fold Claude 2): the 4 existing `test_dogfood_doc_sanity.py` substring tests + the `test_shared_templates.py` Two-tier parametrize must all keep passing — substring constraints listed inline in Bucket E test table; preserve-tokens added to Scope items 4/5/8/9. | `tests/test_bootstrap_cli.py`, `tests/test_dogfood_doc_sanity.py` |
