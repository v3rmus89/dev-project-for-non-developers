...
- **PR #17 merge status — RESOLVED** (iter-5 fold, Codex Tier-2 PR-#18 review):
  PR #17 **merged into `main` on 2026-05-20**. Re-verified against post-merge
  `main`: PR #17 did NOT modify the v1 `--apply` success block — it added
  `_main_apply_adopt` + adopt-mode code earlier in the file, which shifted the
  v1 block down. The v1 success block is now at **`cli.py:779-800` on main**
  (was 358-378 pre-merge). No rebase conflict; the gh-hint slots cleanly into
  the unchanged `if args.github_review != "none":` block (now line 786).
...
2. **Commit 2**: Tighten CLAUDE.md other sections AND mirror to `shared/CLAUDE.md.tmpl`
   ...
   commit also updates `tests/test_dogfood_doc_sanity.py` lines 126-128 + docstring**
...
| 3 | Tighten `CLAUDE.md` ## Plan review loop section from 34 lines → ~6 lines: keep the rule ("write to `docs/plans/...`, run review-plan-by-codex + claude, stop when no imp-3 remain"), drop the detailed iteration cadence + filename convention + bootstrap-exception. **End with**: "See `docs/plans/README.md` for filename convention, when-to-stop rule, and consistency self-check cadence" — pointing at the existing canonical surface, NOT duplicating to CONTRIBUTING.md (iter-1 fold). **(iter-5.5 consistency fold)**: the pointer does NOT name "bootstrap exception" — CLAUDE.md's bootstrap-exception content is dropped outright (iter-3 fold Claude 3), not redirected; README.md's bootstrap exception is a different, non-equivalent thing. | `CLAUDE.md` |
...
In `bootstrap_lib/cli.py`'s `main()`, the v1 `--apply` success block at **lines 779-800
on post-PR-#17-merge main** (iter-5 fold: re-verified against `origin/main` after PR #17
merged 2026-05-20 — was 358-378 pre-merge; PR #17's adopt-mode code shifted it down but
did NOT modify the block itself. The `if args.github_review != "none":` line is now 786,
OAuth-token prints 791-799). Insert the gh-hint detection + print logic **INSIDE the existing `if args.github_review
...
| `test_dogfood_doc_sanity.py` | **MUST be updated in Commit 2** (iter-4 fold Claude 2.2): the `claude`-mode invariant at **lines 126-128** (iter-5 fold — verified against `origin/main`: `assert "@codex review" not in text, (…)` spans 126-128; comment at 125; `claude[bot]` assertion at 123) currently asserts root CLAUDE.md does NOT mention `@codex review`. Scope item 4 adds the `@codex review` keyword to CLAUDE.md (skill repo's dogfood, both bots active — verified via PR #16 reviews). Update the assertion. ALSO reconcile the function's docstring (lines 116-119) — "catches drift between shared/CLAUDE.md.tmpl and the dogfood mirror" no longer holds for the Two-tier section since it intentionally diverges (Scope item 9). The shared-template invariant in `test_shared_templates.py:404` stays unchanged. |
...
- **CLAUDE.md vs CONTRIBUTING.md vs docs/plans/README.md**: three on-demand surfaces.
  ...
  is the canonical home for **plan-loop discipline** (filename convention,
  when-to-stop rule, consistency self-check cadence, human-approval-gate
  wording) — CLAUDE.md POINTS at it for these topics. (CLAUDE.md's own
  bootstrap-exception content is dropped outright, not pointed-at — iter-3 fold
  Claude 3; README.md's bootstrap exception is a different, non-equivalent thing.)
  `CONTRIBUTING.md` is the
...
- **PR #17 merge handling — RESOLVED** (iter-5 fold): PR #17 merged 2026-05-20.
  Re-verified against post-merge `main` — PR #17 did NOT touch the v1 success block;
  it shifted to `cli.py:779-800`. No rebase conflict. A follow-up PR (out of scope
  here, per iter-3 Codex 1) mirrors the gh-hint into adopt-mode's `_main_apply_adopt`.
...
| PR #17's cli.py changes conflict with the gh-hint addition on merge | **RESOLVED — PR #17 merged 2026-05-20** (iter-5 fold). Re-verified against post-merge `main`: PR #17 did NOT modify the v1 success block — it added `_main_apply_adopt` + adopt-mode code earlier, shifting the v1 block to `cli.py:779-800`. No rebase needed; the gh-hint slots into the unchanged `if args.github_review != "none":` block. The adopt-mode gh-hint mirror into `_main_apply_adopt` remains a SEPARATE follow-up PR (iter-3 Codex 1 decision — out of scope regardless of merge order). |
...
| 4-fold | **2 real imp-3** (one Codex imp-3 rejected — see below). (a)-folds: CONTRIBUTING.md:114 pointer NOT changed — framework stays in CLAUDE.md, the planned "below in this document" edit was a broken cross-ref (Codex 1; line cited as 111 in the original iter-4-fold log, corrected to 114 at iter-5.5); same-AI Tier-1 wording fix added to CONTRIBUTING.md 111/190 + tmpl 196/251, closes BACKLOG.md:557 properly (Codex 2 — earlier plan claimed closure via CLAUDE.md edit alone, but CONTRIBUTING.md still had the ambiguity); `gh repo create` visibility = TWO explicit `--private`/`--public` lines instead of shell-metacharacter-unsafe `<--private\|--public>` placeholder (Codex 4); GitLab-remote limitation corrected to "accepted UNDOCUMENTED" — earlier text claimed a CLAUDE.md doc note that no Scope item delivered (Codex 5); Bucket D code lint fixes — drop `f` on no-interpolation prints (F541), `except` tuple simplified to `(OSError, subprocess.SubprocessError)` (Claude 2.1); `test_dogfood_doc_sanity.py` `@codex review` update assigned to Commit 2 + line-ref corrected 125→127-130 + docstring reconciliation (Claude 2.2 + 1.1); both-docs Codex-setup pointer moved OUT of `if not has_remote:` — fires whenever both-docs regardless of remote (Claude 2.3); Two-tier intentional-divergence carve-out added to Gate 10 + Risks + Architecture (Claude 2.4); line-count estimates synced — `shared/AGENTS.md.tmpl` ~109-113, `shared/CLAUDE.md.tmpl` rendered band ~130-145 single figure (Claude 1.2). **Codex 3 REJECTED (c)**: claimed skill-repo dogfood is claude-only (PR template lists only `claude[bot]`) — premise wrong; verified via `gh pr view 16` that PR #16 was reviewed by BOTH `chatgpt-codex-connector` + `claude` bots. The PR template IS stale (missing Codex bot) — flagged as a separate follow-up, out of scope for the IA refactor. Test count 7 → 8 NEW (Claude 2.3 with-remote variant). |
| 4.5 consistency | 3 internal drifts after iter-4 fold: CONTRIBUTING.md line 111 described as both "do NOT update" and "update" (iter-4 conflated lines 111 + 114 — framework pointer is 114, same-AI command is 111); stale evidence-table row still described the superseded iter-2 3-line inline Codex block; iter-3-fold narrative omitted Codex 5. | All folded; line-number disambiguation + wording refresh |
| 4 (tier-2, codex) | 0 imp-3 + 1 imp-2 | merge-eligible after fold — see iter-5 fold below |
| 4 (tier-2, claude) | 0 imp-3 + 2 imp-2 | "ready after minor verification" — see iter-5 fold below |
| 5-fold (Tier-2 PR-#18 review) | **0 imp-3 from both Tier-2 bots.** (a)-folds: PR #17 merged 2026-05-20 → all stale "PR #17 not merged / doesn't exist on main yet" wording refreshed; `cli.py` v1 success block re-verified against post-merge `main` — moved 358-378 → **779-800** (PR #17 didn't modify the block, just shifted it); adopt-mode-exclusion rationale re-pinned to actual merge state (Codex P2). `test_dogfood_doc_sanity.py` `@codex review` line-ref corrected 127-130 → **126-128** — verified against `origin/main` (Claude #1; iter-3's `:125` and iter-4's `127-130` were both wrong). **Claude #2 REJECTED (c)**: "verify `import subprocess`" — the plan ALREADY mandates adding it (Scope item 12 + Bucket D + evidence row Codex iter-2 #3); reviewer missed the existing coverage. |
| 5.5 consistency | 5 drifts after iter-5 fold: (1) bootstrap-exception named in Scope item 3 + Architecture point-at lists though it's dropped-not-redirected — removed from both; (2) evidence row Codex iter-4 #1 mis-cited line-111 for the framework pointer (is 114); (3) iter-4-fold log said line 111 both "NOT changed" and "wording fix added" — "NOT changed" corrected to 114; (4) evidence row Claude iter-4 1.1 stated iter-4's wrong line numbers as fact — reworded as superseded-by-iter-5; (5) Critical-files credited only Scope item 4 for BACKLOG.md:557 closure — Scope item 2's CONTRIBUTING.md edits are the dispositive fix. | All folded as wording/cross-ref corrections |
...
| Codex iter-4 #1 | Plan's CONTRIBUTING.md line-114 edit ("the (a/b/c/d) framework below in this document") is a broken cross-ref — the framework is NOT moving to CONTRIBUTING.md | **(a) fold** | Scope item 2 + Bucket A: line-**114** pointer to CLAUDE.md stays unchanged; planned edit dropped (iter-5.5 consistency fix: was mis-cited as line-111 here — 114 is the framework pointer, 111 is the same-AI command) |
...
| Claude iter-4 1.1 | `test_dogfood_doc_sanity.py` `@codex` assertion line-ref wrong; stale docstring | **(a) fold — iter-4 fix itself imprecise** | iter-4 corrected `:125`→`127-130` (still wrong); **iter-5 re-corrected to 126-128**, verified against `origin/main` (assertion 126-128, comment 125, `claude[bot]` assertion 123 — see "Claude Tier-2 (PR #18) #1" row). Docstring reconciliation added to Scope item 13(ii) |
...
| Codex Tier-2 (PR #18) P2 | Adopt-mode exclusion rationale stale — "doesn't exist on main yet" false now that PR #17 merged; re-validate against actual main | **(a) fold** | iter-5: PR #17 merged 2026-05-20; all "PR #17 not merged" wording refreshed; `cli.py` v1 block re-verified 358-378 → 779-800 on post-merge main; adopt-mode exclusion re-pinned to merge state (scope decision unchanged per iter-3 Codex 1) — pre-loop constraint, Scope item 12, NOT-in-scope, Bucket D, Architecture, Risks, "What we are NOT doing", Critical files |
| Claude Tier-2 (PR #18) #1 | `test_dogfood_doc_sanity.py` `@codex review` line-ref imprecise | **(a) fold** | iter-5: corrected 127-130 → 126-128, verified against `origin/main` (assertion 126-128, comment 125, `claude[bot]` assertion 123) — Scope item 13, Bucket E, Sequencing, Critical files |
| Claude Tier-2 (PR #18) #2 | "Verify `import subprocess` in cli.py" | **(c) reject — already covered** | The plan ALREADY mandates `import subprocess` (Scope item 12 "also add `import subprocess`", Bucket D code comment, evidence row Codex iter-2 #3). Reviewer missed the existing coverage; no plan change. |
...
| 6530e38 | Tightened 6 CLAUDE.md sections (plan-review-loop, two-tier, cross-session, self-improvement, approval-gate, focused-commits) + mirrored to shared/CLAUDE.md.tmpl; flipped `test_dogfood_doc_sanity` Two-tier invariant to allow `@codex review` | none (matches Scope items 3-9, 13(ii) / Bucket B) | Tier-1 imp-2: re-added the `.github/workflows/claude-review.yml` pointer dropped from the CLAUDE.md Two-tier line — folded via `--amend` |
| a41d5a2 | Tightened AGENTS.md Cross-session (5→3) + Self-improvement (10→5) sections; mirrored byte-identically to shared/AGENTS.md.tmpl; AGENTS.md 141→130 lines | Compressed slightly past Bucket C estimates (Cross-session 5→3 vs plan 5→4; Self-improvement 10→5 vs plan 10→6) — Tier-1 verified no reviewer-critical content lost; only plan-named droppable elaboration removed | none |
| 4866265 | gh-repo-create hint added to the v1 `--apply` success path: two-state detection (no `.git/` vs `.git/`-no-remote) via a `git remote` subprocess, safe explicit-staging hint, two-alternative visibility lines, both-docs Codex-setup pointer; `import subprocess` added | 9 tests, not the plan's 8 — Tier-1 imp-2 added a subprocess fail-open coverage test (fail-open is an explicit safety contract the plan's 8-test list missed) | none — `make check` green |
| 50d5a9b | gh-hint git detection: `.git` check switched from `.is_dir()` to `.exists()` so a linked-worktree / `--separate-git-dir` `.git` FILE counts as an existing repo (Tier-2 Codex P2); 2 worktree regression tests added | none (Tier-2 fix) | Tier-1 imp-2 (stray non-gitlink `.git` file false-positive) parked to `BACKLOG.md`; Tier-1 imp-1 (worktree-with-remote coverage) folded |
| 850fe2d | gh-hint repo detection switched from the filesystem `.git` check to `git -C <target> rev-parse --is-inside-work-tree` — handles parent-repo subdir, worktree, `--separate-git-dir`, and stray-`.git`-file uniformly; new `test_gh_repo_hint_target_inside_parent_repo_no_git_init`; fail-open test extended to sabotage `rev-parse` | supersedes 50d5a9b's `.is_dir()`→`.exists()` fix (Tier-2 Codex 2nd-pass P2: parent-repo subdir wrongly got `git init`) and removes the parked stray-`.git`-file BACKLOG entry — both collapse into one authoritative probe | none |
...
- Adopt-mode gh-hint (mirror into `_main_apply_adopt`) — PR #17 merged 2026-05-20, so the code now exists on main; the mirror remains a deliberate separate follow-up PR (iter-3 Codex 1 decision).
...
- `bootstrap_lib/cli.py` lines **779-800** — the v1 post-apply success block where
  gh-hint goes (iter-5 fold: re-verified against post-PR-#17-merge `main`; was
  358-378 pre-merge)
...
- `BACKLOG.md:557` — the same-AI Tier-1 ambiguity entry, **folded by this PR** (Scope item 2's CONTRIBUTING.md 111/190 + tmpl 196/251 same-AI edits are the dispositive fix — iter-4 Codex 2; Scope item 4 tightens the CLAUDE.md wording too but does not by itself close it); marked ✅ Done at impl time.
- `tests/test_dogfood_doc_sanity.py` **lines 126-128** (iter-5 fold — verified against `origin/main`: the `assert "@codex review" not in text, (…)` statement spans 126-128; line 125 is its comment; line 123 is the `claude[bot]` assertion. iter-3 cited `:125` and iter-4 cited `127-130` — both wrong) — the dogfood invariant that asserts root CLAUDE.md does NOT mention `@codex review`; must be updated to ALLOW the mention (positive Codex-bot description in Scope item 4 includes `@codex review` keyword). The function docstring (lines 116-119) must also be reconciled — see Scope item 13(ii).
...
- PR #17 (**MERGED 2026-05-20**) — the adoption-mode work; now on `main`. This PR is
  independent; re-verified post-merge that the v1 success block is intact at
  `cli.py:779-800`. A separate follow-up PR mirrors the gh-hint into `_main_apply_adopt`.
