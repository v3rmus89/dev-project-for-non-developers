...
| 63952fd | **Tier-2 codex P2 fold**: scrub `KEEP_THREAD_JSONL` from the V-13.5 verifier's subprocesses — pure `subprocess_env()` helper; `_run` passes `env=` for all make/codex calls — so an exported value can't leak into the seed recipe and spuriously fail the step-3 `jsonl_absent` gate; +2 unit tests (strip + copy-not-mutate + `KEEP_V13_5_JSONL` preserved) | none | Codex Tier-2 (PR #33) caught a real footgun on the merge-blocker (the recipe's shell-level `KEEP_THREAD_JSONL` test isn't covered by run-with-clean-env's EXACT_DROP; didn't affect the passing live run, which had nothing exported). Tier-1 (fresh Claude subagent) confirmed the fix closes the leak end-to-end, no regression from `env=` (all env keys preserved minus the one), mutation-tested the new tests. Claude Tier-2's 3 findings all rejected on premise-check (fallback-masking disproven empirically; fixture ids agree; cleanup race N/A for a manual tool). `make check` green (871 passed/4 skipped) |
| 4d54439 | **Tier-2 codex P2 (PR #33), PR-3 code folded in**: line-anchor Markdown heading matching in `extract_heading_section` (both copies — `bootstrap_lib/section_extract.py` + `scripts/propagate-shared-rules.py` + byte-identical template) and `_replace_section`, replacing the substring `text.count()` that miscounted an inline prose mention of a heading as a duplicate (spurious "found N times"); +2 non-vacuous regression tests | none (PR-3 fix folded into PR #33 per driver decision, not a PR-1 deliverable) | Tier-1 (fresh Claude subagent) confirmed via DIFFERENTIAL old-vs-new execution that output is byte-identical for real full-line headings (incl. production CLAUDE.md/AGENTS.md), both copies agree (cross-check test), `_replace_section` output preserved, the regression tests fail on old code, and the fix also corrects a latent prefix-substring-match bug |
| a65497d | **Tier-2 codex P2 (PR #33), PR-3 code folded in**: register `propagate-shared-rules.py` in `_SCRIPT_TEMPLATE_PAIRS` (byte-identity) + `SHARED_TEMPLATES_TO_SCAN` — it ships via `SHARED_TEMPLATE_MAP` but was unguarded against dogfood↔template drift | none (PR-3 fix folded into PR #33) | Correctly NOT added to `_EXECUTABLE_SCRIPTS` (that list is review-recipe-exec'd scripts only; propagate isn't one); script+template already byte-identical so the new guard passes immediately. Tier-1 confirmed. `make check` green (876 passed/4 skipped) |
