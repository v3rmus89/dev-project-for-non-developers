# C4 — sync `call-details`' plan-review loop with the skill (full catch-up)
...
(`64d94bd`) and PR #38 (`93366c0`). C4 was deferred because `call-details` had a
dirty, actively-changing tree at the time (the post-2b implementation, 15+
modified files).

**Why now.** As of 2026-06-10 a read-only check shows `call-details` is
...
**What the check also revealed (the real scope).** `call-details` was
...
catch-up** scope: re-sync `call-details`' whole plan-review subsystem with the
skill's current state — adapting, not blindly overwriting, the places where
`call-details` deliberately diverged (its PII hard-rules, its Codex orientation,
its `CLAUDE.md` structure).

This is well-motivated: `call-details`' own 19-iteration post-2b doc-layout loop
...
**Regression safety.** C4's artifacts land in `call-details`, not this skill
repo. Verification is therefore *manual in `call-details`*: its own `make check`
...
The outcome is "`call-details`' plan-review loop gains the same drift-guardrails,
...
| # | Item | Source (skill) → Target (`call-details`) |
|---|------|------------------------------------------|
| 1 | **Fact-check pre-pass** | render `shared/scripts-extract-plan-facts.py.tmpl` + `shared/scripts-verify-plan-facts.py.tmpl` → call-details' `scripts/{extract,verify}-plan-facts.py`; add `review-plan-fact-check-by-{codex,claude}` targets + `FACT_CHECK_*`/`PLAN_FACT_CHECK_OUT_*` **and the shared `$(KEY)` var** (added here — Bucket A lands before Bucket B, which reuses it); `test_review_plan_fact_check.py` lands with this bucket (rewritten call-details-local per Bucket G's boundary) |
...
| 3 | **C1 calibration prose** | fold the strict imp-3 calibration block into `call-details`' `review-plan-by-{codex,claude}` prompts (lands together with the JSON-fence upgrade — same prompt string) |
| 4 | **C2 + C3 + cap-consistency rule docs** | propagate into call-details' `CONTRIBUTING.md` + `docs/plans/README.md` ("Stopping the loop") — mechanical where headings already match, hand-aligned where they do not |
| 5 | **`review` dispatcher** | add the `review` MODE/ACTOR target (PR #35) |
| 6 | **Reviewer-surface reconciliation (`CLAUDE.md` + `AGENTS.md`)** | hand-adapt the skill's plan-review-loop / triage / two-tier / human-approval-gate guidance into `call-details`' `CLAUDE.md` (`## Process discipline`), AND assess + align `call-details`' `AGENTS.md` (the GitHub/Codex reviewer-bot surface) against the source AGENTS — **preserving** its `## Hard rules — PII`, Codex orientation, and any call-details-specific guidance. (call-details' AGENTS.md already has *partial* dispatcher + calibration coverage — this is an align-deltas pass, not a wholesale rewrite.) |
| 7 | **Test parity + green check** | **owns the rewrite-boundary rule for ALL ported tests** (no `SKILL_ROOT`/`bootstrap.py`/skill-fixture dependency) + **lands the cross-cutting tests** `test_makefile_review_targets.py` (introspect call-details' **live** Makefile/scripts; drop `_bootstrap_fixture`) and `test_review_loop_artifacts.py` (adapt to call-details' target set) + the **final green-gate**. Feature-specific tests land with their bucket (A: `test_review_plan_fact_check.py`; B: `test_loop_status.py`), each obeying this boundary. Confirm `call-details`' `make check` stays green |
...
| `test_triage_byte_identity.py` / `test_shim_cli_help_consistency.py` / `test_github_review_modes.py` | Skill-internal dogfood / shim / github-mode tests with no call-details analogue |
...
- **Lands:** in call-details' `scripts/` — `extract-plan-facts.py` + `verify-plan-facts.py`
  ...
  (rewritten call-details-local per Bucket G's boundary).
- **Verified at impl:** `make review-plan-fact-check-by-codex PLAN_FILE=<a call-details plan>`
...
- **Lands:** call-details' `scripts/loop-status.py`; `loop-status` / `loop-ack` /
...
- **Lands:** into call-details' `CONTRIBUTING.md` — C2 ("Keep executable runbooks
  out of the plan"), C3 ("Second trigger — same-class regeneration"), and "Cap
  the consistency self-check"; into call-details' `docs/plans/README.md`
...
- **Verified at impl:** `make review MODE=commit ACTOR=codex` dispatches to
  `review-commit-by-codex` (call-details is Codex-oriented).
...
- **`CLAUDE.md` lands:** the skill's plan-review-loop / triage-(a/b/c/d) /
  ...
  call-details' existing `## Process discipline` section structure — *not* a
  section-overwrite.
- **`AGENTS.md` lands (FN2):** an **align-deltas** pass over call-details'
  ...
  calibration to current. call-details' AGENTS.md already carries *partial*
...
- **Owns the rewrite-boundary rule (FN5) for ALL ported tests:** no ported test
  ...
  call-details' `make check` fails on a literal port. Each is rewritten
  call-details-local (introspect call-details' live Makefile/scripts; tmp copies
...
- **Lands the cross-cutting tests** (these need the full target set, so they come
  ...
  - `test_makefile_review_targets.py` — drop its `SKILL_ROOT`/`bootstrap.py`
    `_bootstrap_fixture`; assert against call-details' **live** Makefile/scripts.
  - `test_review_loop_artifacts.py` — adapt artifact-path assertions to
    call-details' target set.
- **Feature-specific tests land with their bucket** (each obeying the rule above):
  ...
  call-details' `scripts/`; carry local fixtures); `test_loop_status.py` →
  Bucket B (portable, no skill deps).
- **Final green-gate:** `call-details`' full `make check` (lint + product
  test-suite + all new review-loop tests) is green.

## Architecture decisions

1. **Plan home = skill repo for the review loop; a frozen copy lands in
   `call-details` for implementation (FN1).** The skill has the *superior* review
   ...
   converged plan is committed verbatim into call-details'
   `docs/plans/2026-06-10-c4-sync-call-details-plan-review-loop.md` as a frozen
   reference copy** — landed as the FIRST call-details commit. This resolves the
   cross-repo Tier-1 binding gap: `make review-commit-by-codex PLAN_FILE=docs/plans/2026-06-10-…md`
   and `make status` in call-details then bind to a *local* plan file, and
   **implementation-log rows are appended to the call-details copy** (where the
   ...
   iteration/evidence; call-details: implementation). **Alternative:** author
   entirely in call-details — rejected, its review machinery is the outdated thing
   C4 exists to fix. **Overrule me if** you'd rather author in call-details despite
   the weaker loop.

2. **Adapt, don't overwrite, the reviewer surfaces `CLAUDE.md` + `AGENTS.md`
   (Bucket F).** call-details diverged both on purpose (PII hard-rules, Codex-first).
   A blind section-overwrite would clobber product-specific safety guidance. We fold
   the skill's process guidance *into* call-details' structure (and align AGENTS'
...
3. **Cross-repo fact-roots for iter-0.5 (verified behavior).** The plan
   ...
   skill checkout and the call-details checkout. Confirmed empirically (iter-0.5
   ...
   `## Critical files` section segregates skill-vs-call-details refs under explicit
...
4. **Clean-tree precondition + own branch + git-revert rollback** (C4's original
   safety contract). Implementation starts only when `call-details`' tree is
   ...
   = call-details HEAD at impl start (currently `f341cd0`; re-pin at impl start as
   it may advance).

5. **Tier-1 during call-details implementation uses call-details' machinery, bound
   to the local frozen plan copy (FN1).** Each focused commit in call-details is
...
6. **Stdlib-only invariant for the fact scripts** carries downstream unchanged —
   the rendered call-details copies must not import `bootstrap_lib` or any
...
| Ported tests break call-details' `make check` (skill-local `bootstrap.py`/`SKILL_ROOT` deps) | Bucket G rewrites each test call-details-local (FN5); hard rule: no skill-repo dep; gated on green `make check` before catch-up is declared done |
...
| Cross-repo Tier-1 review can't bind to a skill-repo plan (FN1) | Decision 1: a frozen plan copy is committed into call-details as the FIRST commit; Tier-1 + `make status` bind to it locally; impl-log rows live there |
...
| call-details' clean-tree precondition regresses mid-impl | Re-check `git status` at impl start; `CODE_ROLLBACK_BASE` pinned; each bucket its own commit for granular revert |

## Verification

- **Per-bucket:** each new/upgraded target smoke-runs in call-details (fact-check
  emits JSON; `loop-status` reports; `review` dispatches; the four rule strings
  present).
- **End-to-end:** run one real cross-review iteration on a *call-details* plan
...
- **Green gate:** `call-details`' `make check` (lint + product tests + ported
...
- **Cross-repo binding (FN1):** after the frozen plan copy lands,
  ...
  `make status` in call-details both resolve it locally (no "PLAN_FILE not found").
...
**Impl-start preflight (FN4):** confirm call-details' tree is clean and the skill
checkout is at `SOURCE_SYNC_BASE` (`18a64be`); pin `CODE_ROLLBACK_BASE` =
call-details HEAD.

Suggested order (each bucket = one focused commit in `call-details`, Tier-1
reviewed via call-details' machinery; buckets after step 1 are largely independent):

1. **Frozen plan copy (FN1)** — commit the converged plan verbatim into
   call-details' `docs/plans/`, FIRST, so every later bucket's Tier-1 can bind to
   it locally.
...
|------|---------|-----|----------|------------------------------------|
| 1 | FN1 — cross-repo Tier-1 plan-binding undefined | 2 | (a) fold | Premise verified. Decision 1 rewritten: frozen plan copy committed into call-details as first commit; Tier-1 + `make status` bind locally; impl-log lives there. Also Decision 5, Implementation rollout step 1, Risks, Verification. |
| 1 | FN2 — AGENTS.md reviewer surface omitted/stale | 2 | (a) fold | Premise *partially* overstated (call-details/AGENTS.md has partial coverage, not "lacks"). Folded valid core: Bucket F renamed to cover CLAUDE.md + AGENTS.md (align-deltas); Scope item 6, Verification, Risks updated. |
...
**CODE_ROLLBACK_BASE** (call-details HEAD at impl start): _pending — pin at impl start (currently `f341cd0`)._
Implementation-log rows live in the **call-details** frozen copy (decision 1); this skill-repo table tracks bucket landings for cross-reference.
...
- `.claude/commands/dev-review.md` — the tracked Claude-Code command deliberately
  NOT ported (FN3); explains why call-details gets no `/dev-review`
...
**In `call-details` (the target — current drifted state):**
...
- /Users/sandeep/Desktop/Code/Boxette/call-details (the target repo — current drifted state; cross-repo, see architecture decision 3)
