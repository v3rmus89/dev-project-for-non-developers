...
- **PR #6** ✅ uv support for Python — `--package-manager={uv,pip}` flag, default `uv` for greenfield, auto-detect for adoption. Non-package mode for greenfield uv (no `[build-system]`); `astral-sh/setup-uv@v8.1.0` in generated CI; `uv sync --locked` strict-lock enforcement; pre-commit framework stays the hook engine in both modes. Co-landed the `--permission-mode plan` bug fix in `make review-plan-by-claude` / `review-plan-consistency-by-claude` (BACKLOG f).
